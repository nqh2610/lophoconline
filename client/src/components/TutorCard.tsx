...
export function TutorCard({
  ...
  return (
    <Card className="h-full flex flex-col hover-elevate overflow-visible" data-testid={`card-tutor-${id}`}>
      <CardContent className="p-4 sm:p-6 flex-1">
        <div className="flex gap-4 mb-4">
          ...
          <div className="flex-1 min-w-0">
            <Link href={`/tutor/${id}`}>
              <h3 className="font-semibold text-lg truncate hover:text-primary transition-colors cursor-pointer mb-2" data-testid={`text-tutor-name-${id}`}>
                {name}
              </h3>
            </Link>
      ...
      <CardFooter className="p-4 sm:p-6 pt-0 flex items-center justify-between gap-3 flex-wrap">
